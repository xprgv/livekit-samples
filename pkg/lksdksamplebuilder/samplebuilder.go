...
func (s *SampleBuilder) Push(p *rtp.Packet) {
	...
	// data := map[uint32]int{}
	// for _, pack := range s.packets {
	// 	if pack.packet != nil {
	// 		// fmt.Println(pack.packet.Timestamp)
	// 		if v, ok := data[pack.packet.Timestamp]; ok {
	// 			data[pack.packet.Timestamp] = v + 1
	// 		} else {
	// 			data[pack.packet.Timestamp] = 1
	// 		}
	// 	}
	// }
	...
	// fmt.Println("----------")
	// for k, v := range data {
	// 	fmt.Println("timestamp:", k, " packets: ", v)
	// }
...
func (s *SampleBuilder) popSample(force bool) (*media.Sample, uint32) {
	...
	// fmt.Println("sample len:", len(packets))
	...
	// fmt.Println("sample len:", len(packets), " have errors:", unMarshalErr)
	if unMarshalErr != nil {
		log.Println("ERROR IN DEPACKETIZER:", unMarshalErr)
		return nil, 0
	} else {
